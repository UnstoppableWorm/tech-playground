...
TRUNCATE TABLE olap.medical_history;

DROP INDEX IF EXISTS olap.medical_history_occurred_at_idx;

-- 준비 단계의 안정성을 위해 100만 행씩 autocommit한다. 이 크기는 측정 대상이 아니다.
...
FROM generate_series(1, 100000000, 1000000) AS ranges(range_start)
\gexec
