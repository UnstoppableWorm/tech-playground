...
public record AggregationPredicateInput(
		...
		String lower,
		String upper,
		List<AggregationPredicateInput> children,
		String name,
		List<AggregationFilterArgumentInput> arguments) {

	public AggregationPredicateInput {
	...
	private FilterExpression toExpression(Budget budget, int depth) {
		...
		return switch (operator) {
			case AND, OR -> new FilterExpression.Junction(
					operator == AggregationPredicateOperator.AND ? LogicalOperator.AND : LogicalOperator.OR,
