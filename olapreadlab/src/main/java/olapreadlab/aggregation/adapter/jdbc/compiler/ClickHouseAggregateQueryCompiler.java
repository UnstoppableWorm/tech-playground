...
@Component
public class ClickHouseAggregateQueryCompiler implements AggregateQueryCompiler {

	@Override
	public StorageBindingKey storageKey() {
		return JdbcStorageKeys.CLICKHOUSE;
	}
	...
	public CompiledQuery<AggregateResultProjection> compile(
		...
		for (int index = 0; index < definitions.size(); index++) {
			...
			var alias = "__m" + index;
			// SummingMergeTree parts may be unmerged, so every read re-sums stored measures.
