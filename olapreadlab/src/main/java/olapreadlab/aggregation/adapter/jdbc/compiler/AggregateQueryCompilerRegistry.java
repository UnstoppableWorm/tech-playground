...
@Component
public class AggregateQueryCompilerRegistry {

	private final List<AggregateQueryCompiler> compilers;

	public AggregateQueryCompilerRegistry(List<AggregateQueryCompiler> compilers) {
	...
	private AggregateQueryCompiler select(
			StorageBindingKey storageKey, ResolvedAggregationQuery query) {
		var candidates = compilers.stream()
				...
				.sorted(Comparator.comparingInt(AggregateQueryCompiler::priority).reversed())
				.toList();
		if (candidates.isEmpty()) {
			throw new IllegalStateException("No aggregate query compiler for " + storageKey.value());
		}
		if (candidates.size() > 1 && candidates.get(0).priority() == candidates.get(1).priority()) {
			throw new IllegalStateException("Ambiguous aggregate query compilers for " + storageKey.value());
		}
